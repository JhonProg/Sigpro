...
(function($) {
    ...
    $.fn.fixedTable = function(opts) {
        ...
        adjustSizes(options);
        resizeWindons(options);
        return tbl;
    }
    
    function resizeWindons(options) {
    	$(window).resize(function() {
    		var layout = $("#" + options.outerId+ " .fixedArea");
    		var w = $(layout).width() - options.fixedColumnWidth - 16 - ($(window).width()-$(layout).width());
			$(".fixedContainer", layout).width(w);
    ...
    function buildFixedColumns(src, section, cols, target) {
        ...
        if ($(section, src).length) {        	
        	...
        	table.addClass($(src).attr("class"));
        	
            var colHead = table.appendTo(target);
            ...
            $(section + " tr", src).each(function() {
                ...
                $(cellType + ":lt(" + cols + ")", this).each(function() {
                	if(cellType == "th"){
                		$("<th>" + $(this).html() + "</th>").addClass(this.className).attr("id", this.id).appendTo(tr);
                	}else{
        ...
        if ($(section, src).length) {
            ...
            $(section + " tr", src).each(function() {
                ...
                $(cellType, this).each(function() {
                	var tdn = null;
                	if(cellType == "th"){
                		tdn= $("<th>" + $(this).html() + "</th>").addClass(this.className);
                	}else{
                		tdn= $("<td>" + $(this).html() + "</td>").addClass(this.className);
                	}
                	$(tdn).attr('colSpan', ($(this).attr('colSpan')) == null ? 1 : $(this).attr('colSpan'));
                	$(tdn).appendTo(tr);
                });
    ...
    function adjustSizes(options) {
        ...
        $("#" + Id + " ." + options.classColumn + " .fixedTable table tbody tr").each(function(i) {        	        	
        	var maxh = 0;
        	var fixedh = $(this).outerHeight(true);
            var contenth = $("#" + Id + " .fixedContainer .fixedTable table tbody tr").eq(i).outerHeight(true);    
            ...
            $(this).height(maxh, true);
            $("#" + Id + " .fixedContainer .fixedTable table tbody tr").eq(i).height(maxh, true);
        });
